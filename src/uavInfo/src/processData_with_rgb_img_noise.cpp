...
/*double RGB_contrast; //image contrast
int RGB_brightness;  // image brightness */
...
static cv::Mat RGB_dst(resize_rgb_width, resize_rgb_height, CV_8UC3, cv::Scalar::all(0));
static cv::Mat RGB_dst_origin(resize_rgb_width, resize_rgb_height, CV_8UC3, cv::Scalar::all(0));
...
void addSaltAndPepperWholeImg(cv::Mat& src, float percetage)
{
    int channels = src.channels();
    int nRows = src.rows;  
    int nCols = src.cols*channels;//图像的总列数 
    //判断图像的连续性
    if (src.isContinuous()) nCols = nRows * nCols, nRows = 1;
    
    for (int i = 0; i < nRows; ++i)
    {
        for (int j = 0; j < nCols; ++j)
        {
            float flag = rand()%100/(double)101;
            if (flag > percetage)
            {
                continue;
            }
...
void imageCb_d(const sensor_msgs::ImageConstPtr& depth_msg)
{
    ...
    try
    {
        ...
        if (rand()%100 / (double)101 < 0.15f) //rotation transformation
        {
            ...
            if (rand()%100 / (double)101 < 0.3f) // translation transformation
            {
...
cv::Mat addRGBGuassianNoise(cv::Mat& srcImage, double miu, double sigma)
{
    ...
    //判断图像的连续性
    if (resultImage.isContinuous()) nCols = nRows * nCols, nRows = 1;
    ...
    for (int i = 0; i < nRows; ++i)
    {
        ...
        {
            //add gaussian noise
            int val = resultImage.ptr<uchar>(i)[j] + generateGaussianNoise(miu, sigma) * 16;
            if (val < 0)
                val = 0;
            if (val > 255)
                val = 255;
...
void ctrlContrastBrightness(cv::Mat& src, double contrast, int brightness)
{
    // cv::Mat new_image = Mat::zeros( src.size(), src.type());
    for( int y = 0; y < src.rows; y++ )
    {
        for( int x = 0; x < src.cols; x++ )
        {
            for( int c = 0; c < 3; c++ )
            {
                src.at<cv::Vec3b>(y,x)[c] = cv::saturate_cast<uchar>( contrast*( src.at<cv::Vec3b>(y,x)[c] ) + brightness );
            }
        }
    }
}
...
void HSV_hue_saturation_value(cv::Mat& src_hsv, double h_ratio, double s_ratio, double v_ratio)
{
    for( int y = 0; y < src_hsv.rows; y++ )
    {
        for( int x = 0; x < src_hsv.cols; x++ )
        {
            src_hsv.at<cv::Vec3b>(y,x)[0] = cv::saturate_cast<uchar>( h_ratio*( src_hsv.at<cv::Vec3b>(y,x)[0] ) );
            src_hsv.at<cv::Vec3b>(y,x)[1] = cv::saturate_cast<uchar>( s_ratio*( src_hsv.at<cv::Vec3b>(y,x)[1] ) );
            src_hsv.at<cv::Vec3b>(y,x)[2] = cv::saturate_cast<uchar>( v_ratio*( src_hsv.at<cv::Vec3b>(y,x)[2] ) );
        }
    }
}
...
void callbackRGB(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    try 
    {
        float random_blur = rand()%1000/(double)1001;
        float random_contrast_brightness = rand()%1000/(double)1001;
        ...
        // HSV change
        cv::Mat HSV_img(resize_rgb_width, resize_rgb_height, CV_8UC3, cv::Scalar::all(0));
        cvtColor( RGB_dst, HSV_img, CV_BGR2HSV );
        double h_rand = rand() % 200 / (double)201 + 0.5;
        double s_rand = rand() % 200 / (double)201 + 0.5;
        HSV_hue_saturation_value(HSV_img, h_rand, s_rand, 1.0);
        cvtColor( HSV_img, RGB_dst, CV_HSV2BGR );
        if (rand() % 2 !=0)
        {
            // gaussian blur
            cv::Mat gblur = RGB_dst.clone();
            cv::GaussianBlur(RGB_dst, gblur, cv::Size(3, 3), 1.5, 1.5);
            RGB_dst = gblur;
        }
        // gaussian and pepper
        RGB_dst  = addRGBGuassianNoise(RGB_dst, 0.5, 0.4);

        double pepper_percent_RGB = rand() % 5 / (double)100;
        addSaltAndPepperWholeImg(RGB_dst, pepper_percent_RGB);

        double rd_indx_contrast = rand() % 1000 / (double)1001 * 0.8f + 0.6f;
        int noise_bright = rand() % 4 - 1;
        ctrlContrastBrightness(RGB_dst, rd_indx_contrast, noise_bright);
