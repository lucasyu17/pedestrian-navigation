...
void ContourGaussianNoise(cv::Mat& image_m, cv::Mat& dst_depth_bg_pepper, cv::Mat& depth_image_uint8, cv::Mat& dst_depth_origin)
{
    ...
    cv::imshow("dst_depth_origin", dst_depth_origin);
    cv::waitKey();
    ...
    cv::drawContours(contours, contours_bin, -1, 255, noise_width/2.0);
    ...
    cv::imshow("dst_depth_bg_pepper", dst_depth_bg_pepper);
    cv::waitKey();
...
{       
    std::default_random_engine generator;
    std::normal_distribution<double> distribution_x(0.0, resize_rgb_height/18.0);
    std::normal_distribution<double> distribution_y(0.0, resize_rgb_width/18.0);
    int nc = src.cols;
    int nr = src.rows;
