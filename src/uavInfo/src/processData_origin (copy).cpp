...
void CallbackPointCloud(const sensor_msgs::PointCloud2ConstPtr& cloud)
{
    ...
    for (int i_pt=0; i_pt<cloud_in->points.size(); ++i_pt) {
        ...
        float intensity;
        if (point.data_c[0]==1.f)
        {
            intensity = point.data_c[0] * 0.5f;
        }
        else
        {
            intensity = point.data_c[0];
        }
        intensity = intensity/7.f;

        float x_origin, y_origin, x_rotate, y_rotate;
        x_origin = point.data[0] - position_odom_x;
        y_origin = point.data[1] - position_odom_y;
        float yaw_current_true;
        yaw_current_true = yaw_current * 3.15f;
        rotate_x_y(x_origin, y_origin, x_rotate, y_rotate, yaw_current_true);
    
        int x_tmp;
        x_tmp = int(x_rotate * 5 + 0.5f);
        int y_tmp;
        y_tmp = int(y_rotate * 5 + 0.5f);
        int z_tmp;
...
void callbackRGB(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    char c_b[20] ,c_g[20] ,c_r[20];
    int count_pixel = 0;
    try 
    {
        cv_ptr = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::BGR8);
        cv::Mat dst;
        cv::resize(cv_ptr->image, dst, cv::Size(256, 192));
        ...
        // cout << endl << "there is: " << count_pixel << "pixels in the image.." << endl;
