...
bool last_step_is_at_cross = false;
...
void callbackRestFlag(const std_msgs::Float64& msg)
{   
    reset_flag = msg.data;
...
{ 
    ...
    adj_vecs.push_back({1, 4, 6, 9});
    adj_vecs.push_back({2, 5, 7, 10});
    adj_vecs.push_back({3, 6, 11});
    adj_vecs.push_back({4, 9, 12});
    adj_vecs.push_back({5, 8, 10, 13});
    adj_vecs.push_back({6, 9, 11, 14});
    adj_vecs.push_back({7, 10, 15});
    adj_vecs.push_back({8, 13});
    adj_vecs.push_back({9, 12, 14});
    adj_vecs.push_back({10, 13, 15});
    adj_vecs.push_back({11, 14});
    ...
    path_file.open("/home/ubuntu/chg_workspace/log/training/path_1.csv");
    ros::Subscriber control_signal_sub = n.subscribe("/mobile_base/commands/velocity", 1, controlSignalCallback);
    control_signal_file.open("/home/ubuntu/chg_workspace/log/training/control_1.csv");

    ros::Subscriber sub_reset_flag = n.subscribe("/reset_flag",1,callbackRestFlag); 
    ...
    cout << "got_new_position: " << got_new_position << endl;

    while(ros::ok())
    {
        /* Close detection: to find the closest point */
        // while (!got_new_position)
        // {
        //     cout << "not received new position!" << endl;
        //     Delay(50);
        // }
        // got_new_position = false;
        bool reached_any_point = false;
        int close_index = 0;

        for (int indx=0; indx<16; ++indx)
        {
            double dist_x = sqrt(
                (all_points[indx][0] - position[0])*(all_points[indx][0] - position[0]) 
              + (all_points[indx][1] - position[1])*(all_points[indx][1] - position[1]));
            if (dist_x < close_dist)
            {
                reached_any_point = true;
                close_index = indx;
                break;
            }
            if (indx == 15)
            // have not reached any cross point
            {
                last_step_is_at_cross = false;
            }
        }
        ...
        if(reached_any_point && !last_step_is_at_cross)
        {   
            cout << "current position ground truth: " << position[0] << ", " << position[1] << endl;
            cur_position_index = close_index;
            // cout<< " current close distance: "<< close_dist<<endl;
            cout << "You reached the target position! at: " << all_points[cur_position_index][0] << ", " << all_points[cur_position_index][1] << endl;
            ...
            reached_any_point = false;
            close_index = 0;
            last_step_is_at_cross = true;
